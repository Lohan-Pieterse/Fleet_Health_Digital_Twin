const { insertHeartbeat, getLatestHeartbeatByHost, getLatestHeartbeatAllHosts, getHeartbeatHistory } = require('../models/heartbeat_Model');
...
const handleIngestHeartbeat = async (req, res) => {
  try {
    ...
    if (!host || !timestamp || cpu_load == null || mem_used_mb == null || !ip) {
      return res.status(400).json({ error: 'host, timestamp, cpu_load, mem_used_mb and ip are required' });
    }

    await upsertHost(host, ip);
    const heartbeat = await insertHeartbeat(host, timestamp, cpu_load, mem_used_mb, services ?? []);

    return res.status(201).json({ data: heartbeat });
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', event: 'ingest_heartbeat', message: error.message }));
...
const handleGetLatestHeartbeatByHost = async (req, res) => {
  try {
    ...
    if (!heartbeat) {
      return res.status(404).json({ error: `No heartbeat found for host ${hostId}` });
    }

    return res.status(200).json({ data: heartbeat });
  } catch (error) {
    console.error(JSON.stringify({ level: 'error', event: 'get_latest_heartbeat_by_host', message: error.message }));
    return res.status(500).json({ error: 'Internal server error' });
  }
};
...
const handleGetLatestHeartbeatAllHosts = async (req, res) => {
  try {
    ...
    console.error(JSON.stringify({ level: 'error', event: 'get_latest_heartbeat_all_hosts', message: error.message }));
    return res.status(500).json({ error: 'Internal server error' });
  }
};
...
const handleGetHeartbeatHistory = async (req, res) => {
  try {
    ...
    console.error(JSON.stringify({ level: 'error', event: 'get_heartbeat_history', message: error.message }));
    return res.status(500).json({ error: 'Internal server error' });
  }
};
